...
public class TaskDetailActivity extends AppCompatActivity {
        ...
        if (exec != null) {
            ...
            if (now > exec + threeDaysMs && "ACTIVE".equals(status)) {
                ...
                if (userId != null) {
                    ...
                    task.setCategoryName(doc.getString("categoryName"));

                    String difficulty = doc.getString("difficulty");
                    String importance = doc.getString("importance");
                    try {
                        if (difficulty != null) task.setDifficulty(Task.Difficulty.valueOf(difficulty));
                    } catch (Exception ignored) {}
                    try {
                        if (importance != null) task.setImportance(Task.Importance.valueOf(importance));
                    } catch (Exception ignored) {}

                    StatisticsManagerService statsManager = new StatisticsManagerService();
                    statsManager.updateStatisticsOnTaskStatusChange(userId, task, Task.Status.ACTIVE);
                }
    ...
    private void markDone() {
        ...
        String oldStatusStr = (String) currentData.get("status");
        Task.Status oldStatus = Task.Status.ACTIVE;
        try {
            if (oldStatusStr != null) oldStatus = Task.Status.valueOf(oldStatusStr);
        } catch (IllegalArgumentException ignored) {}

        final Task.Status finalOldStatus = oldStatus;
        ...
        taskRef.update(updates).addOnSuccessListener(aVoid -> {
            ...
            if (userId != null) {
                ...
                String category = (String) currentData.get("categoryName");
                task.setCategoryName(category != null ? category : "Default");
                ...
                StatisticsManagerService statsManager = new StatisticsManagerService();
                statsManager.updateStatisticsOnTaskStatusChange(userId, task, finalOldStatus);
            }

            String userId2 = (String) currentData.get("userId");
            String difficulty = (String) currentData.get("difficulty");
            String importance = (String) currentData.get("importance");

            if (userId2 == null) {
                userId2 = (String) currentData.get("createdBy");
            }
            ...
            if (importance != null && importance.equals("SPECIAL")) {
                ...
                allowed = 1;
                limitKind = 1;
                sinceTs = System.currentTimeMillis() - TimeUnit.DAYS.toMillis(30);
            ...
            if (hasLimit) {
                final String finalUserId = userId2;
                ...
                awardXPToUser(userId2, difficulty, importance);
                loadTask();
            }
